...
assert(app.includes("Authorization: getSupabaseAuthorizationHeader(authMode)"), "Supabase REST does not use dynamic auth header");
assert(app.includes("player_team_roster"), "player team roster RPC is missing");
assert(app.includes('supabaseSelect("teams", "active=eq.true&select=id,name,slug,active&order=name.asc", { authMode: "player" })'), "player login must load active teams directly from the teams table in player auth context");
